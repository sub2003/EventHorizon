...
public class EventService {
    ...
    public boolean updateEvent(String eventId, String title, String category,
                               ...
                               String description, double summaryPrice, Connection conn) throws SQLException {

        String sql = "UPDATE events SET title=?, category=?, date=?, time=?, venue=?, description=?, ticket_price=? WHERE event_id=?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public boolean updateEventWithImage(String eventId, String title, String category,
                                        ...
                                        String imageType, double summaryPrice, Connection conn) throws SQLException {

        String sql = "UPDATE events SET title=?, category=?, date=?, time=?, venue=?, description=?, ticket_price=?, image_data=?, image_type=? WHERE event_id=?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, title);
            ps.setString(2, category);
            ps.setString(3, date);
    ...
    public boolean deleteEvent(String eventId) {
        ...
        try {
            ...
            try (PreparedStatement ps1 = conn.prepareStatement("DELETE FROM event_ticket_types WHERE event_id=?")) {
                ps1.setString(1, safeTrim(eventId));
                ps1.executeUpdate();
            }

            try (PreparedStatement ps2 = conn.prepareStatement("DELETE FROM events WHERE event_id=?")) {
                ps2.setString(1, safeTrim(eventId));
                if (ps2.executeUpdate() <= 0) { conn.rollback(); return false; }
            }

            conn.commit();
            return true;
        } catch (SQLException e) { e.printStackTrace(); if (conn != null) try { conn.rollback(); } catch (SQLException ignored) {} return false; }
    ...
    private Event mapRowToEvent(ResultSet rs) throws SQLException {
        ...
        try { event.setImageData(rs.getBytes("image_data")); } catch (SQLException ignored) {}
        try { event.setImageType(rs.getString("image_type")); } catch (SQLException ignored) {}
        return event;
    }
