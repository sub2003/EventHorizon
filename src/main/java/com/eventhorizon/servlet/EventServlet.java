...
public class EventServlet extends HttpServlet {
    ...
    /** Handles creating a brand-new event. Uses replaceTicketTypes (safe: no existing rows). */
    private void handleAdd(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            // replaceTicketTypes is safe here: brand-new event, no bookings exist
     ...
     *
     * KEY FIX: reads ticketTypeId[] from hidden form fields and calls
     * updateTicketTypes() instead of replaceTicketTypes().
     * This preserves ticket_type_id values (no FK violations) and keeps
     * available_seats correctly adjusted for existing bookings.
     */
    private void handleUpdate(HttpServletRequest req, HttpServletResponse resp)
        ...
        // ticketTypeId[] — populated by hidden inputs in editEvent.jsp.
        // Empty string for brand-new rows added via the "+ Add Ticket Type" button.
        String[] typeIds    = req.getParameterValues("ticketTypeId");
        String[] typeNames  = req.getParameterValues("typeName");
        String[] typePrices = req.getParameterValues("typePrice");
        String[] typeSeats  = req.getParameterValues("typeSeats");
        ...
        try {
            ...
            if (imagePart != null && imagePart.getSize() > 0) {
                ...
                ok = eventService.updateEventWithImage(
                        eventId, title, category, date, time, venue, description,
                        imageData, imageType,
                        summaryPrice,
                        conn);
            } else {
            ...
            // KEY FIX: use updateTicketTypes (not replaceTicketTypes) so that:
            //   - existing types are UPDATEd in place (ticket_type_id preserved)
            //   - new types are INSERTed
            //   - removed types are DELETEd safely (FK errors are caught per-row)
            //   - available_seats for existing types reflects real bookings
            ticketTypeService.updateTicketTypes(eventId, typeIds, typeNames, typePrices, typeSeats, conn);
